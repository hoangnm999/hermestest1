...
RUN printf '#!/command/with-contenv sh\nCONFIG="$HERMES_HOME/config.yaml"\nif [ -f "$CONFIG" ]; then\n  echo "[019-set-model] Setting deepseek endpoint"\n  sed -i "s/^  default:.*/  default: deepseek-v4-flash/" "$CONFIG"\n  sed -i "s/^  provider:.*/  provider: deepseek/" "$CONFIG"\n  if ! grep -q "api_base.*api.deepseek" "$CONFIG"; then\n    sed -i "/^model:/a\  api_base: https://api.deepseek.com/v1" "$CONFIG"\n    echo "[019-set-model] Added api_base for DeepSeek"\n  fi\nfi\n' > /etc/cont-init.d/019-set-model && chmod +x /etc/cont-init.d/019-set-model
