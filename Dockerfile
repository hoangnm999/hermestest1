...
# Debian trixie's bundled nodejs is pinned to 20.x
# which reached EOL in April 2026 — we copy node + npm + corepack from the
# upstream node:22 image instead so we can stay on a supported LTS without
# waiting for Debian 14 (forky, ~mid-2027).
# Bookworm-based slim image used
# so the produced binary links against glibc 2.36, which runs cleanly on
# our Debian 13 (trixie, glibc 2.41) runtime.
# Bumping to a new Node major
...
# ran as PID 1. See #15012.
# Phase 2 of the s6-overlay supervision plan
...
# and per-profile gateways.
# /init becomes PID 1 below — see ENTRYPOINT.
#
...
# we map between them inline.
# The noarch + symlinks tarballs are
...
# build and leave dead bytes in the cache.
# A single curl + arch-keyed URL
# is simpler and cache-friendlier.
#
# Supply-chain integrity: every tarball is checksum-verified against the
# upstream-published SHA256.
# To bump S6_OVERLAY_VERSION, fetch the four
# `.sha256` files from the corresponding release and update the ARGs.
# The
...
RUN set -eu; \
    ...
    # 'Hermes WebUI' catalog).\
    # The image has moved to s6-overlay /init\
    ...
    # crash with 'tini: No such file or directory' on startup.\
    # The shim\
    ...
    # ENTRYPOINT.\
    # Safe to drop once the affected catalogs are updated.\
    ln -sf /init /usr/bin/tini

# Non-root user for runtime;
# UID can be overridden via HERMES_UID at runtime
RUN useradd -u 10000 -m -d /opt/data hermes

COPY --chmod=0755 --from=uv_source /usr/local/bin/uv /usr/local/bin/uvx /usr/local/bin/

# Node 22 LTS: copy the node binary plus the bundled npm + corepack JS
# installs from the upstream image.
# npm and npx are recreated as symlinks
...
# ui-tui/package.json.
# Copying the tree up front lets npm resolve the
...
# This is the default since npm 10+, which is
# what the image ships now (via the node:22 source stage).
# We set it
...
# runtime `npm install` that then failed with EACCES.
# Keeping the env
...
# change.
# Before this split the Python install sat after `COPY .
# .`, so
...
# excluded from the build context by .dockerignore's `*.md`.
# uv's build
# frontend stats the readme path during dep resolution, so we `touch` an
# empty placeholder — the real README is restored by `COPY .
# .` below.
#
...
# without a first-boot lazy install.
...
# Python/image combinations without usable wheels.
# The Docker image is
# Linux-only, so keeping the native libolm/build-toolchain packages here
# avoids the cross-platform failures that kept [matrix] out of [all]
# while still making Matrix work in the published container.
# Fixes #30399.
#
...
# `a+rX,go-w`
# gives the non-root hermes user read + traverse but no write;
# root retains
...
# cached layer above;
# `--no-deps` makes this a fast egg-link creation with no
...
# tree), NOT into $HERMES_HOME.
# $HERMES_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install;
# stamping it at boot used to clobber that
# host install's marker and wrongly block its ``hermes update``.
# A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume.
# Each supervised service then drops to the hermes user via
# `s6-setuidgid hermes` in its run script.
# If HERMES_UID is unset, services
...
# hermes_cli/build_info.py read it at runtime.
# Both `hermes dump` and
# banner.get_git_banner_state() try the baked SHA first, then fall back
# to live `git rev-parse` for source installs (unchanged behaviour).
#
# The arg is optional — local `docker build` without --build-arg simply
# omits the file, and the runtime falls back to live-git lookup.
# CI
...
# the profile create/delete hooks (Phase 4);
# they live under
